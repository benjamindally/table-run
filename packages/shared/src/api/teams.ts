...
export const teamsApi = {
  ...
  addCaptain: (teamId: number, playerId: number, token?: string) =>
    api.post<Team>(`/teams/${teamId}/add_captain/`, { player_id: playerId }, token),

  /**
   * Transfer captaincy to another player
   */
  transferCaptain: (teamId: number, newCaptainId: number, token?: string) =>
    api.post<Team>(`/teams/${teamId}/transfer_captain/`, { new_captain_id: newCaptainId }, token),

  /**
   * Remove a captain
   */
  removeCaptain: (teamId: number, playerId: number, token?: string) =>
    api.post<Team>(`/teams/${teamId}/remove_captain/`, { player_id: playerId }, token),
